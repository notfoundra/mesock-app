...
function loadProductionData(idaps, start, stop, target, est) {
    if (!idaps) return;
    $.ajax({
        url: base_url + '/planning/getQtyProduksiDanBSByStyle?idaps=' + idaps + '&start=' + start + '&stop=' + stop + '&target=' + target + '&est=' + est,
        method: 'GET',
        dataType: 'json',
        success: function (response) {
...
$(document).ready(function () {
    $('#btn-lanjut-planning').on('click', function () {
        ...
        // Load via AJAX
        const url = `${base_url}planning/planningpagev2/${orderId}/${id_pln_mc}?iframe=1`;
