...
<a href="<?= site_url('tasks/daily') ?>" class="text-sm text-secondary mb-3 d-inline-block">&larr; Kembali ke Checklist Harian</a>

<div class="card mb-4">
    <div class="card-header">
        <h6 class="mb-0"><?= esc($task['title']) ?></h6>
        <p class="text-sm text-secondary mb-0"><?= esc($task['task_date']) ?></p>
    </div>
    <div class="card-body">
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="mb-0">Lampiran File</h6>
    </div>
    <div class="card-body">
        <form action="<?= site_url('tasks/daily/' . $task['id'] . '/evidence') ?>" method="post" enctype="multipart/form-data" class="row g-2 mb-3">
            <?= csrf_field() ?>
            <div class="col-md-4">
